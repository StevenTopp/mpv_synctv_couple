...
* **Web 指令控制 Native**：实现极高精度的 `AndroidBridge` 控制，Web 端网页接收到房间内 WebSocket 的同步播放指令后，能实时控制本地原生 `libmpv` 执行高精度播放、暂停、毫秒级进度跳转 (Seek)。
* **Native 状态反馈 Web**：在原生播放器控制条上的任意操作（如双击播放/暂停、拉动进度条），会自动通过桥接方法逆向反馈给内置网页，并分发至整个房间，实现完全对称的指令广播。

### 3. ⚡ 毫秒级播放倍速完美同步 (Playback Speed Sync)
* **原生倍速联动**：完美支持房间内任意倍速调整指令（如 1.25x, 1.5x, 2.0x 等）。
* **零延迟速度切换**：通过 AndroidBridge 直接调用原生 `player.playbackSpeed` 参数，切换流畅自然，避免了网页端加速播放时容易产生的音频变调、音画不同步和画面卡顿问题。

### 4. 🔄 智能时序对齐与自动追赶机制 (Timeline Auto-Catchup)
* **毫秒级时序监控**：后台高频比对原生播放器 `time-pos` 与网页房间内共享的时间戳，当因网络卡顿、设备解码延迟等产生轻微时间偏差时，自动执行本地自适应追赶。
* **主动重构对齐**：当偏差超出阈值（如 1.5 秒）时，会自动触发高精度的原生 Seek 操作，强行拉回并校准至房间核心时间轴，确保全员帧级对齐。

### 5. 🛡️ 第三方直链防盗链及自定义头部注入 (Custom HTTP Headers)
...
### 6. 🎇 高性能硬件加速弹幕渲染层 (Native Danmaku Canvas)
